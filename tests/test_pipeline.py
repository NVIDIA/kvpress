...
def test_pipeline_fa2(compression_ratio, kv_press_llama3_1_flash_attn_pipeline):  # noqa: F811
    context = "This is a test article. It was written on 2022-01-01."
    questions = ["Repeat the last sentence"]
    press = ExpectedAttentionPress(compression_ratio=compression_ratio)
    cache = DynamicCache()
    answers = kv_press_llama3_1_flash_attn_pipeline(
    ...
    kv_press_llama3_1_flash_attn_pipeline.model.set_attn_implementation("sdpa")
    press = ExpectedAttentionPress(compression_ratio=compression_ratio)
    cache = DynamicCache()
    answers_sdpa = kv_press_llama3_1_flash_attn_pipeline(
        context, questions=questions, press=press, cache=cache, max_new_tokens=6
    )["answers"]
    kv_press_llama3_1_flash_attn_pipeline.model.set_attn_implementation("flash_attention_2")
