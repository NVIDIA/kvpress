...
from evaluate_registry import DATASET_REGISTRY, SCORER_REGISTRY, PRESS_REGISTRY
...
class EvaluationRunner:
        ...
        if isinstance(press, DuoAttentionPress):
            ...
            for ps in press.presses:
                if isinstance(ps, ThinKPress):
                    ...
                    logger.info(f"Set ComposedPress (ThinKPress component) key_channel_compression_ratio to {key_channel_compression_ratio}")
                else:
                    ...
                    if hasattr(ps, 'compression_ratio'):
                        ...
                        logger.info(f"Set ComposedPress ({ps.__class__.__name__} component) compression_ratio to {compression_ratio}")
                    else:
                        logger.warning(f"ComposedPress component {ps.__class__.__name__} has no 'compression_ratio' attribute.")
    ...
    def _setup_model_pipeline(self):
        ...
        if device == "auto":
            ...
            self.pipeline = pipeline("kv-press-text-generation", model=model_name, device=device, model_kwargs=model_kwargs)
    ...
    def _run_inference(self):
        ...
        for context, df_group in tqdm(df_context_grouped, total=self.df["context"].nunique(), desc="Running Inference"):
            ...
            max_new_tokens = max_new_tokens_cfg if max_new_tokens_cfg is not None else df_group["max_new_tokens"].iloc[0]
    ...
    def _get_save_filenames(self, output_dir: Path) -> Tuple[Path, Path]:
        ...
        return output_dir / f"{self.predictions_prefix}_{filename_stem}.csv", output_dir / f"{self.metrics_prefix}_{filename_stem}.json"
    ...
    def run_evaluation(self):
        ...
        if predictions_filename.exists() and metrics_filename.exists():
            logger.info(f"Evaluation files already exist at \n {predictions_filename} \n {metrics_filename}. \nSkipping evaluation.")
            return
