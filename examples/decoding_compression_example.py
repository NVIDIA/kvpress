...
def main():
    ...
    try:
        result = pipeline(context=context, question=question, press=decoding_press, max_new_tokens=20)
    ...
    try:
        result_random = pipeline(
            context=context, question="How is the weather?", press=decoding_press_random, max_new_tokens=15
        )
    ...
    prefilling_press = StreamingLLMPress(
        compression_ratio=0.4, n_sink=4  # Remove 40% during prefilling  # Keep 4 sink tokens
    )
